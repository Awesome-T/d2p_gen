...
import 'package:exap/models/enums.dart';
    ...
    static DTOCorpus toDTO (Corpus enumField) {
      try {
        return DTOCorpus.values.firstWhere((e) => e.name.toUpperCase() == enumField.name.toUpperCase());
...
$trace""");
    ...
    static Corpus fromDTO(DTOCorpus enumField) {
      try {
        return Corpus.values.firstWhere((e) =>
...
$trace""");
    ...
    static DTOErrorCodeExeptions toDTO (ErrorCodeExeptions enumField) {
      try {
        return DTOErrorCodeExeptions.values.firstWhere((e) => e.name.toUpperCase() == enumField.name.toUpperCase());
        } on FormatException catch (e, trace) {
          throw FormatException("""Error
${e.source}
${e.message}
$trace""");
    ...
    static ErrorCodeExeptions fromDTO(DTOErrorCodeExeptions enumField) {
      try {
        return ErrorCodeExeptions.values.firstWhere((e) =>
          e.name.toUpperCase() == enumField.name.toUpperCase());
      } on FormatException catch (e, trace) {
        throw FormatException("""Error
${e.source}
